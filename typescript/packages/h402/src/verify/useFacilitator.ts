...
import {FacilitatorConfig, FacilitatorResponse, PaymentRequirements, SettleResponse, VerifyResponse,} from "../types";
...
export function useFacilitator({url}: FacilitatorConfig) {
  async function makeRequest<T>(
    ...
    try {
      ...
      console.error(
        `[ERROR-PAYMENT-FLOW] Facilitator connection error: ${error instanceof Error ? error.message : String(error)}`,
      );
      return {
        data: null,
